...
const Table = ({ headers, rows, emptyMessage, striped = false, roundedCorners = true }: {headers: string[], rows: object[], emptyMessage: string, striped?: boolean, roundedCorners?: boolean}) => { 
    if (!rows || rows.length === 0) {
        return <div className="bg-white dark:bg-gray-900 rounded-xl shadow-md p-6 text-center text-gray-900 dark:text-gray-100">{emptyMessage}</div>; 
    }
