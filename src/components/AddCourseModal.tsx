...
import { CourseSubmit } from '@/types/data';

interface AddCourseModalProps {
  isOpen: boolean;
  onClose: () => void;
  // Common select-course props
  courses: {
    id: string;
    name: string;
    title?: string;
  ...
  // Optional admin props for creating courses
  allowCreate?: boolean;
  onAddCourse?: (course: CourseSubmit) => void;
}
...
const AddCourseModal: React.FC<AddCourseModalProps> = ({
  ...
  const [mode, setMode] = useState(allowCreate ? 'create' : 'select');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (onAddCourse) {
      const newCourse: CourseSubmit = {
        courseCode,
        courseName,
        courseDesc,
        courseUnit: parseInt(courseUnit, 10),
        semester: semester ? parseInt(semester, 10) : 0,
        status,
      };
      ...
      >
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-3xl font-bold">
            {allowCreate && mode === 'create' ? 'Create a New Course' : 'Select a Course'}
          </h2>
        ...
        {allowCreate && mode === 'create' ? (
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="relative">
                <input
                  type="number" id="semester" value={semester} onChange={(e) => setSemester(e.target.value)}
                  className="peer w-full px-4 py-3 bg-white/10 rounded-lg border border-white/30 focus:outline-none focus:ring-2 focus:ring-primary-accent placeholder-transparent"
                  placeholder="Semester" required
                  min="1"
                />
                <label htmlFor="semester" className="absolute left-4 -top-3.5 text-sm text-white/80 transition-all peer-placeholder-shown:text-base peer-placeholder-shown:text-white/60 peer-placeholder-shown:top-3.5 peer-focus:-top-3.5 peer-focus:text-sm peer-focus:text-white/80 bg-card-background px-1">Semester</label>
              </div>
            </div>
            <div className="relative">
              <select
                ...
                className="peer w-full px-4 py-3 bg-white/10 rounded-lg border border-white/30 focus:outline-none focus:ring-2 focus:ring-primary-accent placeholder-transparent text-black dark:text-white"
                required
              >
                <option value="active">Active</option>
                <option value="inactive">Inactive</option>
              </select>
              <label htmlFor="status" className="absolute left-4 -top-3.5 text-sm text-white/80 transition-all peer-placeholder-shown:text-base peer-placeholder-shown:text-white/60 peer-placeholder-shown:top-3.5 peer-focus:-top-3.5 peer-focus:text-sm peer-focus:text-white/80 bg-card-background px-1">Status</label>
            </div>
              ...
              {filteredCourses.map(course => (
                ...
                >
                  ...
                  <p className="text-sm text-white/80">
                    {course.id} {course.semester !== undefined && `• ${course.semester}${course.semester === 1 ? 'st' : course.semester === 2 ? 'nd' : 'th'} Semester`}
                  </p>
                </div>
              ))}
            </div>
